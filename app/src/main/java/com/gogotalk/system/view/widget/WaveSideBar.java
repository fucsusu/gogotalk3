...
public class WaveSideBar extends View {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mOldPosition != mNewPosition) {
                    if (mNewPosition >= 0 && mNewPosition < mLetters.size()) {
                        ...
                        if (mListener != null) {
                            mListener.onLetterChange(mLetters.get(mNewPosition).charAt(0));
                        }
                    }
                }
                invalidate();
    ...
    private void drawWavePath(Canvas canvas) {
        mWavePath.reset();
        // 移动到起始点
        mWavePath.moveTo(0, mCenterY - 3 * mRadius);
        //计算上部控制点的Y轴位置
        int controlTopY = mCenterY - 2 * mRadius;

        //计算上部结束点的坐标
        int endTopX = (int) ( mRadius * Math.cos(ANGLE) * mRatio);
        int endTopY = (int) (controlTopY + mRadius * Math.sin(ANGLE));
        mWavePath.quadTo(0, controlTopY, endTopX, endTopY);

        //计算中心控制点的坐标
        int controlCenterX = (int) (1.8f * mRadius * Math.sin(ANGLE_R) * mRatio);
        ...
        mWavePath.quadTo(0, controlBottomY, 0, controlBottomY + mRadius);
    ...
    private void drawCirclePath(Canvas canvas) {
        //x轴的移动路径
        mCircleCenterX =    (2.0f * mRadius + 2.0f * mCircleRadius) * mRatio-mCircleRadius;
